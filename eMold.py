...
def create_Бибки_debug():
    global пирожки
    #пирожки = 6 #40 # ааааа, эээээ ну тама, типо, колво рядков и столбов сколько клеток спавнить, да, вроде, та в пизду
    n= 0
...
def render_branch_a(x,y,cell_id=0):
    ...
    col = (64, 64, 64)
    width = int(block_size/5) +1 # +1 потому что 1+1 ты не один  #5
...
def render_links_debug_LEGACY(x,y,cell_id): # ЛЕГАСИ
    global block_size
    # c - center
    xc = x + int((block_size / 2))
    yc = y + int((block_size / 2))
...
def render_field():
    ...
    WINDOW_WIDTH = field_screen[0] + field_screen[2]
    WINDOW_HEIGHT = field_screen[3]+field_screen[1]
        ...
        case 1:
            for x in range(offset_x // block_size, ((WINDOW_WIDTH + offset_x) // block_size)):
                for y in range(offset_y // block_size,
                    ...
                    render_x -= offset_x
                    render_y -= offset_y
                    if (x < 0 or y < 0): # хуюшка от беззымянника УДАРЬ МЕНЯ
                        continue
...
def hudo_render(): # рендер худа
    global panel_btns
    global panel_btn_selected
    global panel_xy
    global panel_colours
    global panel_size
    global SCREEN
    pygame.draw.rect(SCREEN, panel_colours["base"], [panel_xy[0], panel_xy[1], panel_size[0], panel_size[1]])
    for col in range(panel_col_row[0]):
       for row in range(panel_col_row[1]):
            btn_tmp = panel_btns[col][row]
            pygame.draw.rect(SCREEN,panel_colours["base"],btn_tmp["rect"])
            pygame.draw.rect(SCREEN, panel_colours["outline"], btn_tmp["rect"], 4)
            text_tmp = font_button.render(btn_tmp["label"], True, (25,25,25) )
            text_rect = text_tmp.get_rect(center=( btn_tmp["rect"].x + btn_tmp["rect"].width / 2, btn_tmp["rect"].y + btn_tmp["rect"].height / 2))
            SCREEN.blit(text_tmp, text_rect)
            #SCREEN.blit(text_tmp, (btn_tmp["rect"].x,btn_tmp["rect"].y))
           # pygame.draw.rect(SCREEN, (255,5,5), panel_btns[col][row]["rect"])



def hudo_handle(): # хендлер худа
    global hudo_mouse
    hudo_mouse = pygame.mouse.get_pos()
...
def render(): # Рендерит поле, и клетки на нём
    ...
    if show_button == True:  # Если кнопки показываются - рисуем
        ...
        # Render modes
        render_cells_mode_0 = create_button((225, 225, 255), 50, 50, 'R1', 0, 100, 10, 10)
        render_cells_mode_1 = create_button((225, 225, 255), 50, 50, 'R2', 50, 100, 10, 10)
        render_cells_mode_2 = create_button((225, 225, 255), 50, 50, 'R3', 100, 100, 10, 10) # genome
    ...
    for event in pygame.event.get():  # Выход при нажатии на крестик
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            if show_button == True:  # Если кнопки показываются, то проверяем жмяканье
                ...
                # Это костыльное смена рендера
                if render_cells_mode_0.collidepoint(event.pos):
                    render_cells = 3  # на нормальный с дивжением камеры
                if render_cells_mode_1.collidepoint(event.pos):
                    render_cells = 4  # на упрощённый с дивжением камеры
                if render_cells_mode_2.collidepoint(event.pos):
                    render_cells = 101  # на упрощённый с дивжением камеры
...
# Временный костыль для чертова, ибо мне лень чинить рендер, я пока что напишу с нуля для дебага
def panel_create_button(col,row,id=0,label="none"): # label - , id - айди для кейса, вот оно фактически определяет функцию кнопки
    global panel_col_row
    global panel_size
    global panel_xy
    global panel_btn_size

    btn_size = panel_btn_size#(panel_size[0]/panel_col_row[0], panel_size[1]/panel_col_row[1])
    print("btn_size", btn_size)
    btn_xy = (panel_xy[0]+btn_size[0]*col,panel_xy[1]+btn_size[1]*row)
    print("btn_xy", btn_xy)
    #input()
    btn_rect = pygame.Rect(btn_xy[0],btn_xy[1], btn_size[0], btn_size[1])

    btn = {"id": id, "label":label, "rect": btn_rect, "push": False, "toggle": False}
    # hold - если зажать кнопку, будет ли она продолжать быть нажатой, или сразу же отожментся
    # toggle - по нажатию переключает кнопку
    # по стандарту все кнокпи это pulse
...
def create_button(color, width, height, mes, x, y, textX, textY): # color - цвет в rgb ((int, int,int)), width - ширина (int), height - высота (int), mes - текст на кнопке (str), x - координата по x (int),                                 # y - координата по y (int), react - реакция при клике (str)
    ...
    if x <= pygame.mouse.get_pos()[0] <= x + width and y <= pygame.mouse.get_pos()[1] <= y + height: # проверка находится ли мышь на кнопке, можно сделать как проверке на нажатие, но я не хочу
            pygame.draw.rect(SCREEN, color_light, button)#[x, y, width, height])
    else:
...
# КЛЕТИ И ПОЛЕ
fieldSize = 60 # высота\широта игр.поля
...
# ГЕНОМ
adult_genome_len = 17 # длинна генома взлойслой клетки\ кол-во генов у взпрослого
seed_genome_len = 5 # длинна генома семечка\ кол-во геном у семечка
total_genome_len = adult_genome_len+ seed_genome_len # длинна генома в целом, УДАРЬ МЕНЯ, тут костыльное решение на пока где первое n генов это для взрослого, а остаток это для семечка
gene_len = 18 # длинна каждого отдельног гена
...
genomesCols = [(255,0,0) for i in range(cells_len)] # цвета для окраски по геному, яхз каким образом это отдельный массив и почему он сущетсвует
пирожки = 6 # кол-во рядкво и столбоцов спавна бибок
...
render_traverse_mode = 1 # режимы проходки, 1 = рендер через прямую проходку поля, 2 = рендер через проходку клеток (хорошо когда всё поле на экране)
render_cells = 3 # 1 - приколы от подсмешка, 3 - упрщенный рендер
# 100 - рендер от генома
debug_mode = 0 # 0 - no, 1 - спрашивай беззымянника
debug_links = 0
debug_heading = 0
...
interval = 1 # Задержка между обновленями рассчётов (не действует на рендер)
...
field_screen = (250,0,WINDOW_WIDTH-500,WINDOW_HEIGHT) # x,y, width, height
hudo_screen = (field_screen[2],0,WINDOW_WIDTH-field_screen[2],WINDOW_HEIGHT) # x,y, width, height
...
# худо беззымянника
# тут кста константы\кортежи написаны маленькими ну тупа по приколу шоб веселее было



# ЛЕГАСИ ХУДО, ЗАДЕЛ ПОД ХАРАНИЛИЩЕ КНОПОК
hudo_mouse = (0,0)
hudo_colours = {"mouse":(25,200,25)}


panel_size = (WINDOW_WIDTH-300, 100) # размер панели
panel_xy = (150,WINDOW_HEIGHT-panel_size[1]) # кортеж с коориднатиой левого верхнего угла панели
panel_col_row = (8,1) # количесвтво рядов и колонок конопок в панеле
panel_colours = {"outline": (25,25,25),"base":(200,200,200),"hover":(240,240,240),"pressed":(160,160,160)} # цвета панели

panel_btn_size = ( int(panel_size[0]/panel_col_row[0]) , int(panel_size[1]/panel_col_row[1]) )
panel_btns = [[None for __ in range(panel_col_row[1]) ] for _ in range(panel_col_row[0])] # аэээ, кнопки походу
panel_btn_selected = (-1,-1) # координата выбранной копки (hover) КСТАТТИ ОБРАТИ ВНИМАНИЕ, тут могут быть вообще любые координаты введены, от -4,5 и до 20,600
panel_btn_pressed = False # нажата ли выбраная кнопка

# секвенция создания пустых кнопок панельки
for col in range(panel_col_row[0]):
    for row in range(panel_col_row[1]):
        panel_btns[col][row] = panel_create_button(col,row)
